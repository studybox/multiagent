...
from acktr_disc import learn, evaluate
...
def eval(num_timesteps, seed):
    set_global_seeds(seed)
    num_cpu = 32
    env = SubprocVecEnv([create_env(seed, i) for i in range(num_cpu)], is_multi_agent=True)
    policy_fn = CategoricalPolicy # TODO
    #vf = NeuralNetValueFunction
    evaluate(policy_fn, None,  env, seed = 1, total_timesteps=int(num_timesteps * 1.1), nprocs=num_cpu)
def main():
    logdir = './log_evl2'
