...
class Model(object):
    def __init__(self, policy, obe_space, obn_space, nedges, nnodes, ac_space, nenvs, total_timesteps, nprocs=2, nsteps=200,
        ...
        nbatch = nenvs * nsteps
        self.num_agents = num_agents = len(obe_space)
        ...
        for k in range(num_agents):
            if identical[k]:
                ...
                step_model.append(policy(sess, obe_space[k], obn_space[k], ac_space[k],
                                         nenvs, 1, nstack, nedges[k], nnodes[k], reuse=False, name='%d' % k))
                train_model.append(policy(sess, obe_space[k], obn_space[k], ac_space[k],
                                          nenvs * scale[k], nsteps, nstack, nedges[k], nnodes[k], reuse=True, name='%d' % k))
        ...
        def train(obs, states, rewards, masks, actions, values):
            ...
            for k in range(num_agents):
                ...
                ob_graph = self.convert(obs[k])
                new_map.update({
                    train_model[k].X["X_ns"]: ob_graph[0],
                    train_model[k].X["X_nr"]: ob_graph[1],
                    train_model[k].X["X_e"]: ob_graph[2],
                    train_model[k].X["X_n"]: ob_graph[3],
                    train_model[k].X["e2ns"]: ob_graph[4],
                    train_model[k].X["e2nr"]: ob_graph[5],
                    train_model[k].X["ns2e"]: ob_graph[6],
                    train_model[k].X["b2e"]: ob_graph[7],
                    train_model[k].X["b2n"]: ob_graph[8],
                    train_model[k].X_v["X_ns"]: ob_graph[0],
                    train_model[k].X_v["X_nr"]: ob_graph[1],
                    train_model[k].X_v["X_e"]: ob_graph[2],
                    train_model[k].X_v["X_n"]: ob_graph[3],
                    train_model[k].X_v["e2ns"]: ob_graph[4],
                    train_model[k].X_v["e2nr"]: ob_graph[5],
                    train_model[k].X_v["ns2e"]: ob_graph[6],
                    train_model[k].X_v["b2e"]: ob_graph[7],
                    train_model[k].X_v["b2n"]: ob_graph[8],
        ...
        def step(ob, av, *_args, **_kwargs):
            ...
            for k in range(num_agents):
                ...
                a_v = np.concatenate([multionehot(av[i], self.n_actions[i])
                                      for i in range(num_agents) if i != k], axis=1)
                #print(a_v.shape)
                ob_graph = self.convert(ob[k])
                a_, v_, s_ = step_model[k].step(ob_graph, ob_graph, a_v)
                a.append(a_)
                v.append(v_)
                s.append(s_)
            return a, v, s
        ...
        def value(obs, av):
            ...
            for k in range(num_agents):
                a_v = np.concatenate([multionehot(av[i], self.n_actions[i])
                                      for i in range(num_agents) if i != k], axis=1)
                ob_graph = self.convert(ob)
                v_ = step_model[k].value(ob_graph, a_v)
                v.append(v_)
            return v
    ...
    def convert(self, obs):
        ...
        for i in range(obs.shape[0]):
            ...
            d = obs[i,5]+0.5+obs[i,1]*0.5+0.25
        ...
        e2ns, e2nr, ns2e, b2e, b2n = self._multiones(e2ns, ob_n.shape[0]), self._multiones(e2nr, ob_n.shape[0]), self._multiones(ns2e, ob_e.shape[0]), self._multiones(b2e, ob_e.shape[0]), self._multiones(b2n, ob_n.shape[0])
        return (ob_ns, ob_nr, ob_e, ob_n, e2ns, e2nr, ns2e, b2e, b2n)
    def _multiones(self, values, depth):
...
def learn(policy, expert, env, seed, total_timesteps=int(40e6), gamma=0.99, lam=0.95, log_interval=1, nprocs=128,
          nsteps=1, nstack=1, ent_coef=0.1, vf_coef=0.5, vf_fisher_coef=1.0, lr=0.05, max_grad_norm=0.5,
    ...
    nedges = [6,6]
    nnodes = [6,6]
    #obn_space = env.obn_space
    #obe_space = env.obe_space
    #nedges = env.nedges
    #nnodes = env.nnodes
    ac_space = env.action_space
    num_agents = len(ac_space)

    make_model = lambda: Model(policy, obe_space, obn_space, nedges, nnodes, ac_space, nenvs, total_timesteps, nprocs=nprocs, nsteps=nsteps,
